...
/** Spectate a live Lichess bot-game on piChess's own board.
  *
  * `POST /lichess/games` challenges a random online Lichess bot to a casual 5+3
  * game (our bot account, configured by `LICHESS_BOT_TOKEN`, plays White),
  * creates a *mirror* game in our own game-service, and forks a daemon fiber
  * that follows the public Lichess game stream and replays every move into the
  * mirror via `MakeMove`. The browser then subscribes to the mirror's existing
  * SSE feed (`/api/games/{id}/events`) and renders it read-only — no Lichess
  * board embed, no token in the browser.
  *
  * The public stream (`/api/stream/game/{id}`) needs no auth, so the mirror
  * never competes with the bot process's own play-stream on
...
object LichessSpectate:
  ...
  /** What the browser gets back: the mirror game id to spectate plus the
    * orientation to show it from (our bot plays White).
    */
  ...
  private final case class ChallengeResp(
      id: Option[String],
      challenge: Option[IdHolder]
  ):
  ...
  /** A line from `/api/stream/game/{id}`. The first line is the full game
    * (carries cumulative UCI `moves`); subsequent lines carry the latest move
    * `lm` plus the new `fen`.
    */
  ...
  def routes(
    ...
    Routes(
      ...
      // Spectate an *existing* Lichess game by id (e.g. one surfaced in the
      // unified Spectate list from `GET /api/account/playing`): mirror it and
      // follow the public stream — no challenge, no token needed for the
      // public stream.
      Method.POST / "lichess" / "games" / string("id") / "spectate" ->
        handler { (id: String, _: Request) =>
          spectateExisting(client, id).either.map {
            case Right(resp) =>
              Response.json(resp.toJson)
            case Left(msg) =>
              Response.text(s"lichess spectate: $msg").status(Status.BadGateway)
  ...
  /** Orchestrate: pick opponent → challenge → mirror game → fork the follower
    * fiber → return the mirror id.
    */
  private def start(
      client: ZioGameService.GameServiceClient,
      token: String
  ...
  /** Spectate an already-running Lichess game: just mirror it and fork the
    * follower against the public stream. Orientation defaults to White (the
    * board can be flipped client-side).
    */
  private def spectateExisting(
      client: ZioGameService.GameServiceClient,
      lichessId: String
  ): ZIO[Client, String, StartResponse] =
    for
      mirror <- client
        ...
        .mapError(e => s"mirror newGame failed: ${e.getStatus}")
      _ <- ZIO.logInfo(
        s"Lichess spectate existing $lichessId, mirror ${mirror.gameId}"
      )
      _ <- follow(client, lichessId, mirror.gameId).forkDaemon
    yield StartResponse(mirror.gameId, "white")
  ...
  /** Challenge `bot` to a casual 5+3 standard game; our bot plays White so the
    * mirror's default orientation matches our side. The returned challenge id
    * is also the game id once accepted.
    */
  private def challenge(
    ...
    liPostForm(
      ...
      body.fromJson[ChallengeResp].toOption.flatMap(_.gameId) match
        ...
        case None =>
          ZIO.fail(s"challenge to $bot was not created: ${body.take(180)}")
    }

  // -- Mirror follower ------------------------------------------------------

  /** Follow the public Lichess game stream and replay each new move into the
    * mirror game. Retries the stream open until the opponent accepts (the
    * endpoint 404s until the game exists).
    */
  private def follow(
      client: ZioGameService.GameServiceClient,
      lichessId: String,
    ...
    val streamOnce =
      ZIO.scoped {
        for
          url <- ZIO.fromEither(
            URL
              .decode(s"$LichessBase/api/stream/game/$lichessId")
              .left
              .map(_.getMessage)
          )
          resp <- Client.request(Request(method = Method.GET, url = url))
          _ <- ZIO.unless(resp.status.isSuccess)(
            ZIO.fail(s"stream not ready (${resp.status.code})")
          )
          ...
          _ <- resp.body.asStream
            ...
            .filter(_.nonEmpty)
            .runForeach(line =>
              applyLine(client, mirrorId, line, appliedRef, lastFenRef)
            )
        yield ()
      }
    ...
    streamOnce
      ...
      .catchAllCause(c =>
        ZIO.logWarning(
          s"Lichess spectate follow ended for $lichessId: ${c.failureOption.getOrElse(c)}"
        )
      )
      .unit
      ...
      case Some(sl) =>
        appliedRef.get.flatMap { applied =>
          sl.moves match
            ...
            case Some(all) =>
              val toks =
                all.split(' ').iterator.map(_.trim).filter(_.nonEmpty).toVector
  ...
  /** One mirror move. Errors are logged, not propagated — a single bad move
    * shouldn't tear down the whole follower (the spectate just stops tracking;
    * the real game continues on Lichess).
    */
  private def applyMove(
    ...
    client
      ...
      .unit
      .catchAll(e =>
        ZIO.logWarning(s"mirror $mirrorId rejected move $uci: ${e.getStatus}")
      )
