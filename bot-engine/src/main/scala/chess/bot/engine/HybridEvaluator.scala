...
/** Blends two evaluators into one score: `(1 − w)·base + w·other`.
  *
  * Motivation: a strong NNUE strictly dominates a hand-crafted eval (HCE), so
  * production engines go pure-NNUE. But when the NNUE is weaker than the HCE
  * (e.g. small / undertrained), a blend weighted toward the stronger eval can
  * still beat either alone IF the two make *decorrelated* errors — the classic
  * weak-learner-helps- ensemble effect. This evaluator makes that hypothesis
  * testable without touching the search: it's just another [[Evaluator]].
  *
  * `nnueWeight` is the mixing weight on `other` (the NNUE), in `[0, 1]`. 0.0 →
  * pure base (HCE); 1.0 → pure other (NNUE); 0.25 → mostly HCE with a
  * quarter-strength NNUE nudge. Both sub-evals return white-POV centipawns, so
  * the blend is in the same units — no rescaling needed.
  *
  * Cost is the sum of both sub-evals per call (HCE is cheap; the NNUE
  * dominates), so a blend is roughly as expensive as pure NNUE. If that proves
  * too slow for the depth it buys, the next step is a *lazy* blend — skip the
  * NNUE when the HCE is already far outside the search window.
  */
final class HybridEvaluator(
  ...
  private val wEnd =
    if tapered then math.max(0.0, math.min(1.0, nnueWeightEndgame)) else w
  ...
  override def evaluate(state: PositionView): Int =
    ...
    math
      .round((1.0 - ww) * base.evaluate(state) + ww * other.evaluate(state))
      .toInt
    ...
    net match
      case Some(n) =>
        ...
        math
          .round(
            (1.0 - ww) * base
              .evaluate(state) + ww * n.evaluateFrom(acc, state.activeColor)
          )
          .toInt
      case None =>
        evaluate(state)
