...
object FeatureExtractorSpec extends ZIOSpecDefault:
  ...
  private def featuresOf(
      fen: String
  ): ZIO[Any, chess.model.GameError, Map[String, Int]] =
  ...
  def spec = suite("FeatureExtractor.material")(
    ...
    suite("FeatureExtractor.full")(
      test(
        "starting position has zero net PST contribution — perfect symmetry"
      ) {
      ...
      test("bishop_pair is +1 when only white has the pair") {
        ...
        yield assertTrue(
          FeatureExtractor.full.features(state)("bishop_pair") == 1
        )
      },
      ...
      test(
        "bishop_pair is 0 when both sides have ≥ 2 bishops or neither does"
      ) {
        for
      ...
      suite("mobility")(
        ...
        test("knight in the centre has more moves than a corner knight") {
          ...
          for
            central <- chess.codec.FenParserRegex.parse(
              "4k3/8/8/8/3N4/8/8/4K3 w - - 0 1"
            )
            corner <- chess.codec.FenParserRegex
              .parse("4k3/8/8/8/8/8/8/N3K3 w - - 0 1")
          yield assertTrue(
      ...
      suite("passed pawns")(
        test(
          "a white pawn on e5 with no enemy pawns on d/e/f files is passed"
        ) {
        ...
        test(
          "a white pawn blocked by an enemy pawn on the same file is NOT passed"
        ) {
      ...
      suite("pawn weaknesses")(
        ...
        test(
          "connected pawns count when adjacent-file pawn is within ±1 rank"
        ) {
      ...
      suite("knight outpost")(
        test(
          "knight defended by pawn with no enemy pawn attack is an outpost"
        ) {
      ...
      suite("kingless guard")(
        test(
          "missing king bitboard returns no king-safety features (defensive)"
        ) {
