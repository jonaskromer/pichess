...
/** Perft (move-path enumeration) — the gold-standard correctness check for move
  * generation + apply + legality. Counts the leaf nodes of the legal move tree
  * to a fixed depth and compares against the published reference values (Chess
  * Programming Wiki). One test exercises every special move at once: castling,
  * en passant, promotion INCLUDING under-promotions, pins, and check evasion —
  * things a positional spec can miss.
  *
  ...
  * counts (the perft harness will be re-pointed at the copy-make apply, and the
  * reference values are the equivalence proof).
  */
object PerftSpec extends ZIOSpecDefault:
  ...
  /** Count leaves of the legal move tree at `depth`, via the search's own
    ...
    * and the COPY-MAKE apply ([[SearchPos.copyMakeInto]] — returns `false` when
    * the move left the king in check, i.e. illegal, so it's skipped).
    * Re-pointed from the immutable `applyMoveInt` to copy-make: reproducing the
    * published counts is the equivalence proof. Per-ply `SearchPos` + move
    * buffers are indexed by remaining depth (one per recursion level), so a
    * child's generation/apply can't clobber the list its parent is still
    * iterating.
    */
