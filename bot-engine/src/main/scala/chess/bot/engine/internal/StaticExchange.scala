...
/** Static Exchange Evaluator (SEE).
  *
  * Given a capture move `from→to`, computes the net material won (in centipawns
  * from the initiator's POV) assuming both sides recapture with their cheapest
  * remaining attacker until one side has no attackers left or refuses to
  * continue. The classic "swap algorithm" — equivalent to playing out the
  * exchange tree to its forced static conclusion without searching anything.
  *
  * Why this matters for move ordering: pure MVV-LVA can't tell a winning queen
  * capture from a queen sacrifice. A rook capturing a defended bishop looks
  * like +325 cp on the MVV-LVA scale but is actually −175 (rook traded for
  * bishop+pawn). SEE catches that without doing a real recursive search, so
  * it's the cheapest way to rank captures correctly.
  *
  * Includes X-ray detection: when a slider captures and clears the way for a
  * slider behind it (e.g., queen behind bishop), the trailing slider becomes
  * the next attacker. Tracked by removing the just-used attacker from the
  * occupancy bitboard before re-querying the slider-attack masks.
  *
  * Lives in [[internal]] so the search module can call it without exposing it
  * on the public engine surface.
  */
private[engine] object StaticExchange:

  /** Kaufman-ish piece values used by SEE. Matches the values in
    * [[chess.bot.engine.AlphaBetaSearch.pieceValue]] so MVV-LVA ordering and
    * SEE pruning use the same material scale.
    */
  ...
  /** SEE for a capture from `state`. Returns the net centipawn gain from the
    * initiator's POV. Positive = winning capture, 0 = balanced trade, negative
    * \= losing capture.
    *
    * Returns 0 if `move` doesn't actually capture anything (the caller is
    * expected to pre-filter to captures, but the explicit zero short-circuit
    * makes accidental non-capture calls a no-op rather than nonsense).
    */
  ...
  /** Pick the cheapest attacker of `sq` for one side, given the current
    * occupancy. Returns a packed `pieceTypeOrd<<6 | sq`, or `-1` when no
    * attacker. Pawn order: PieceType.King=0..Pawn=5 (matching the enum's
    * ordinal order). Probes lowest-value first (pawn), then knight, bishop,
    * rook, queen, king. Sliders share the bishop/rook ray masks — we query them
    * with the same masks the move generator uses, so the X-ray story is "AND
    * queens with bishop-rays and rook-rays separately" rather than a combined
    * call.
    */
  private inline def cheapestAttacker(
    ...
    if pawnAtk != 0L then
      (5 << 6) | java.lang.Long.numberOfTrailingZeros(pawnAtk)
    else
      val knightAtk = knights & BitboardAttacks.knightAttacks(sq)
      if knightAtk != 0L then
        (4 << 6) | java.lang.Long.numberOfTrailingZeros(knightAtk)
      else
        val bishopRays = BitboardAttacks.bishopAttacks(sq, occ)
        val bishopAtk = bishops & bishopRays
        if bishopAtk != 0L then
          (3 << 6) | java.lang.Long.numberOfTrailingZeros(bishopAtk)
        else
          val rookRays = BitboardAttacks.rookAttacks(sq, occ)
          val rookAtk = rooks & rookRays
          if rookAtk != 0L then
            (2 << 6) | java.lang.Long.numberOfTrailingZeros(rookAtk)
          else
            // Queens move along bishop OR rook rays.
            val queenAtk = queens & (bishopRays | rookRays)
            if queenAtk != 0L then
              (1 << 6) | java.lang.Long.numberOfTrailingZeros(queenAtk)
            else
              val kingAtk = king & BitboardAttacks.kingAttacks(sq)
              if kingAtk != 0L then
                (0 << 6) | java.lang.Long.numberOfTrailingZeros(kingAtk)
              else -1

  /** Inverse of [[PieceType]] ordinal → value, used by the swap loop where we
    * operate on raw Int ordinals to avoid Piece allocations. Keep in sync with
    * [[value]].
    */
