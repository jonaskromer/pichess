...
/** Zero-allocation tapered evaluator for the search hot path.
  *
  * Same scoring formula as [[TaperedEvaluator]] — phase-weighted
  * blend of mg and eg weight tables — but designed to be called
  * thousands of times per [[Search.bestMove]] without allocating:
  *
  *   - Features land in a reusable `Array[Int]` (indexed by
  *     [[FeatureIndex]]), filled by
  *     [[FullFeatures.fillArray]]. No `Map[String, Int]`
  *     constructed per call, no String concat per feature.
  *   - Weights are pre-projected into two parallel `Array[Int]`
  *     (`mgWeights`, `egWeights`) at construction time. Lookup is
  *     an array index, not a `Map.get` + suffix concat.
  *   - The eval loop is a single while over featureCount with one
  *     multiply + one add per `_mg`/`_eg` pair.
  *
  * The reusable buffer is held inside the evaluator — single-
  * threaded by construction. Search is sequential per node so the
  * shared buffer is safe; a future multi-threaded search would
  * give each thread its own evaluator.
  *
  * Score equivalence with [[TaperedEvaluator]] is pinned by a
  * specs check: same weights + position → identical Int score
  * (the rounding happens in the same place, so no FP-vs-integer
  * mismatch).
  */
...
object ArrayTaperedEvaluator:
  ...
  /** Project a `Map[String, Int]` weights snapshot into the two
    * parallel arrays the evaluator expects. The same `_mg` / `_eg`
    * fallback logic as [[TaperedEvaluator]]: when a suffixed key
    * is absent the un-suffixed key is used (so legacy `v1.json`
    * snapshots still play), and missing keys fall back to 0.
    *
    * The projection is the only place where the
    * `Map[String, Int]` → `Array[Int]` translation happens. Build
    * it once at evaluator construction; the hot path then only
    * reads arrays. */
  ...
  /** Same two-step lookup as [[TaperedEvaluator]]'s `weight`
    * helper: prefer the suffixed key, fall back to the un-suffixed
    * key for legacy/non-tapered snapshots, then 0. */
  private def lookupWeight(weights: Map[String, Int], key: String, mg: Boolean): Int =
