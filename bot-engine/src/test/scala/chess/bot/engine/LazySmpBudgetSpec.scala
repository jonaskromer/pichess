...
object LazySmpBudgetSpec extends ZIOSpecDefault:
  ...
  def spec = suite("LazySMP budget")(
    test("ParallelismBudget grabs spare permits non-blocking, caps at available, releases") {
    ...
    test("overlapping budgeted-LazySMP searches stay legal, don't crash, don't leak permits") {
