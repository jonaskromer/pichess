...
/** Loads [[WeightSnapshot]]s from the classpath at engine startup.
  ...
  * classpath. Loading from a resource (rather than a filesystem path the user
  * has to know about) means the weights are baked into the deployable jar —
  * consumers don't need to ship a separate file or configure a path. Tests +
  * dev runs read the same resource the production bot does.
  *
  * Failure modes:
  *   - resource missing → `WeightsLoader.MissingResource`
  *   - JSON malformed → `WeightsLoader.MalformedJson`
  * Both are surfaced as ZIO failures so callers can recover (e.g. fall back to
  * a hardcoded snapshot) rather than aborting startup.
  */
object WeightsLoader:
  ...
  final case class MalformedJson(path: String, reason: String)
      extends RuntimeException(s"weights JSON malformed in $path: $reason")
      with LoadError
  ...
  private def parse(path: String, body: String): IO[LoadError, WeightSnapshot] =
    ZIO
      .fromEither(body.fromJson[WeightSnapshot])
      .mapError(reason => MalformedJson(path, reason))

  /** Write a snapshot to a `Path` as pretty-printed JSON. The companion of
    * [[load]]: tuner runs call this to drop a new version next to the existing
    * weights resources. The path is filesystem-typed (not a classpath resource)
    * so the export step can write into the build's
    * `src/main/resources/weights/` directory and be picked up by the next
    * compile.
    */
  def writeFile(
