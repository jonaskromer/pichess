...
/** Decorator that adds endgame-specific patches on top of the underlying
  * evaluator. The tuner-driven [[ArrayTaperedEvaluator]] captures middlegame
  * and most-endgame positions well but loses resolution in low-piece-count
  * terminal phases where the "drive losing king to the edge / corner" or "pawn
  * can promote before the lone king catches it" patterns dominate the result.
  *
  * Four patterns recognised, additive bonuses to the base eval:
  *   1. KQK / KRK — drive the lone king toward edge / corner. 2. KPK
  *      rule-of-square — pawn promotes before lone king can catch it → huge
  *      bonus (close to queen value). 3. Opposite-color bishops with no rooks /
  *      queens — pull a lopsided material eval back toward draw.
  *
  * All bonuses are white-POV centipawns — same convention as the underlying
  * `Evaluator.evaluate`. The caller (search) flips on black to get side-to-move
  * POV.
  */
final class EndgameAwareEvaluator(inner: Evaluator) extends Evaluator:
  ...
  /** White - black corner-drive bonus when one side has K+Q vs K. Encourages
    * corralling the losing king to a corner where mate is forced in ≤ 10 plies.
    */
  private def kingPlusQueenVsKing(b: BoardLike): Int =
    val wHas =
      b.queensW.nonEmpty && noOtherPieces(b, white = true, exceptQueen = true)
    val bBare = bareKing(b, white = false)
    val bHas =
      b.queensB.nonEmpty && noOtherPieces(b, white = false, exceptQueen = true)
    val wBare = bareKing(b, white = true)
    if wHas && bBare then cornerDriveBonus(b.kingB.lowestBitIdx, white = true)
    else if bHas && wBare then
      -cornerDriveBonus(b.kingW.lowestBitIdx, white = false)
    else 0

  /** Same shape for K+R vs K — drive to the edge (rooks mate via confining to
    * one rank/file).
    */
  private def kingPlusRookVsKing(b: BoardLike): Int =
    val wHas =
      b.rooksW.nonEmpty && noOtherPieces(b, white = true, exceptRook = true)
    val bBare = bareKing(b, white = false)
    val bHas =
      b.rooksB.nonEmpty && noOtherPieces(b, white = false, exceptRook = true)
    val wBare = bareKing(b, white = true)
    if wHas && bBare then edgeDriveBonus(b.kingB.lowestBitIdx, white = true)
    else if bHas && wBare then
      -edgeDriveBonus(b.kingW.lowestBitIdx, white = false)
    else 0

  /** Rule-of-the-square: if the lone king can't catch a passed pawn before it
    * promotes, return a large bonus for the pawn's owner. `dist(king,
    * promoSquare) > dist(pawn, promoSquare)` means the king arrives a tempo too
    * late.
    */
  private def kingPlusPawnVsKing(b: BoardLike): Int =
    val whiteWinning =
      b.pawnsW.popCount >= 1 && noOtherPieces(
        b,
        white = true,
        exceptPawn = true
      ) &&
    ...
    val blackWinning =
      b.pawnsB.popCount >= 1 && noOtherPieces(
        b,
        white = false,
        exceptPawn = true
      ) &&
  ...
  private def ruleOfSquareBonus(b: BoardLike, white: Boolean): Int =
    ...
    val loneKingSq =
      if white then b.kingB.lowestBitIdx else b.kingW.lowestBitIdx
    var rem = pawns
    var bestBonus = 0
    while rem != 0L do
  ...
  /** OCB detected when both sides have ≥1 bishop, opposite colours, and the
    * rest of material is mostly minor pieces (no rooks / queens, since those
    * break OCB drawishness).
    */
  private def oppositeColorBishopsBalanced(b: BoardLike): Boolean =
    val wB = b.bishopsW.raw
    val bB = b.bishopsB.raw
    if java.lang.Long.bitCount(wB) != 1 || java.lang.Long.bitCount(bB) != 1 then
      false
  ...
  /** Chebyshev distance — max of row/col diff. Matches king-move geometry.
    */
  ...
  private def bareKing(b: BoardLike, white: Boolean): Boolean =
    val rest =
      if white then
        b.pawnsW.raw | b.knightsW.raw | b.bishopsW.raw | b.rooksW.raw | b.queensW.raw
      else
        b.pawnsB.raw | b.knightsB.raw | b.bishopsB.raw | b.rooksB.raw | b.queensB.raw
    rest == 0L

  /** True when the side has no pieces other than king + the specified single
    * piece type.
    */
