...
/** Evaluator that combines a [[FeatureExtractor]] with a tuned weight
  ...
  * white-side perspective — the same shape as
  * [[MaterialEvaluator]]'s result, just with weights provided at
  * runtime instead of compiled-in.
  *
  * This is what the Texel-tuner pipeline writes to: train the
  * weights offline → ship them as JSON or via
  * [[chess.bot.data.WeightsRepo]] → load on bot startup → wrap into
  * a [[TunedEvaluator]] → hand it to
  * [[Search.alphaBeta]]. The engine doesn't care how the weights
  * were produced.
  */
