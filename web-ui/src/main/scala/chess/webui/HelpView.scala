  ...
  private val commandsSection: HtmlElement =
    section(
      ...
      table(
        ...
        row("export fen|pgn|json", "Export the current game in the given format"),
        row("undo",               "Undo the last move"),
        row("redo",               "Redo the last undone move"),
        row("draw",               "Claim a draw (50-move rule or threefold repetition)"),
        row("forfeit",            "Resign — the side to move loses; the opponent is the winner"),
  ...
  private val importExportSection: HtmlElement =
    section(
      ...
      p(
        ...
        code("export"), " requires a format argument.",
      ),
      pre(
        className := "help-pre",
...
export json""",
  ...
  private val fenSection: HtmlElement =
    section(
      ...
      p(strong("Format: "), code("<placement> <active> <castling> <en-passant> <halfmove> <fullmove>")),
      p(
        strong("Example: "),
      ...
      dl(
        className := "help-dl",
        dt("Placement"),
        dd("8 ranks separated by /, from rank 8 (top) to rank 1. Letters are pieces (KQRBNPkqrbnp), digits are empty squares. Uppercase = White, lowercase = Black."),
        dt("Active"),
        dd(code("w"), " or ", code("b"), " — whose turn it is."),
        dt("Castling"),
        dd("Combination of ", code("K Q k q"), ", or ", code("-"), " for none."),
        dt("En passant"),
        dd("Target square (e.g. ", code("e3"), ") or ", code("-"), "."),
        dt("Halfmove"),
        dd("Moves since the last pawn push or capture (drives the 50-move rule)."),
        dt("Fullmove"),
  ...
  private val pgnSection: HtmlElement =
    section(
      ...
      p(
        "Move numbers (", code("1."), ", ", code("2."), ", …) and result tokens (",
        code("1-0"), ", ", code("0-1"), ", ", code("*"),
        ") are handled. Comments in ", code("{braces}"), " and NAG annotations (",
        code("$1"), ", ", code("$2"), ") are ignored.",
      ),
    )
  ...
  private val moveNotationSection: HtmlElement =
    section(
      ...
      table(
        className := "help-table",
        row("Coordinate",    code("e2 e4"), " ", code("e2e4"), " ", code("e2-e4"), " ", code("e7 e8=Q")),
        row("Pawn push",     code("e4"), " ", code("d5")),
        row("Pawn capture",  code("exd5"), " ", code("cxb4")),
        row("Piece move",    code("Nf3"), " ", code("Bc4"), " ", code("Rd1"), " ", code("Qd8"), " ", code("Ke2")),
        row("Piece capture", code("Nxf3"), " ", code("Bxc6")),
        row("Disambiguation", code("Nbd2"), " ", code("N1f3"), " ", code("Raxd5"), " (file, rank, or both)"),
        row("Promotion",     code("e8=Q"), " ", code("exd8=R"), " (=Q, =R, =B, =N)"),
        row("Castling",      code("O-O"), " kingside  ", code("O-O-O"), " queenside"),
  ...
  private val rulesSection: HtmlElement =
    section(
      ...
      dl(
        className := "help-dl",
        dt("Pawn"),
        dd("One square forward; two from the starting rank; diagonal capture; en passant; promotion on the back rank."),
        dt("Rook"),
        ...
        dt("King"),
        dd("One square in any direction; castling (", code("O-O"), " / ", code("O-O-O"), ")."),
        dt("Check"),
        dd("Moves leaving your own king in check are rejected. The checked king is highlighted in both TUI and GUI."),
        dt("Castling"),
        dd("King moves two squares toward the rook; rook jumps over. Requires neither piece moved, the path clear, and no check on the king's path."),
        dt("Checkmate"),
        dd("Detected automatically; the game ends and the winner is announced. SAN appends ", code("#"), "."),
        dt("Stalemate"),
        dd("Detected automatically; drawn when the side to move has no legal move but is not in check."),
        dt("50-move rule"),
        dd("Claim a draw with ", code("draw"), " after 50 moves with no pawn push or capture."),
        dt("Insufficient material"),
        dd("Drawn automatically when neither side can checkmate (K vs K, K+B vs K, K+N vs K, K+B vs K+B with same-colored bishops)."),
        dt("Threefold repetition"),
        dd("Claim a draw with ", code("draw"), " when the same position has occurred at least three times."),
