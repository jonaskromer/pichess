...
/** Search-internal MUTABLE board for copy-make.
  *
  * Twelve raw-`Long` piece bitboards + the three cached aggregates,
  * mutated in place. Implements [[BoardLike]] (the read seam the hot
  * NNUE / HCE / move-gen / Zobrist / check readers consume) so it can
  * stand in for an immutable [[chess.model.board.BoardState]] WITHOUT
  * re-materialising one per node — that per-node `BoardState` was ~47 %
  * of depth-6 hybrid allocation. `Bitboard` is an `opaque type … = Long`,
  * so the accessors are zero-cost wrappers the JIT erases.
  *
  * Bit layout is LERF, identical to `BoardState`: bit `i` = square `i`.
  * Lives in `internal` (not `domain`) on purpose — it stays off the
  * domain 100 %-coverage mandate and is exercised by `PerftSpec` + the
  * search specs.
  *
  * Single-threaded by construction: each search thread owns its own
  * per-ply `SearchPos`/`MutableBoard` buffers via `SearchBufs`. */
private[engine] final class MutableBoard extends BoardLike:
  ...
  /** The piece at `pos`, or `None`. Mirrors `BoardState.get` exactly —
    * returns one of the 13 cached `Option[Piece]` flyweights, so it's
    * allocation-free on the move-ordering hot path. */
  def get(pos: Position): Option[Piece] =
    val mask = 1L << pos.squareIdx
    if (pwRaw & mask)      != 0L then Piece.someOf(Color.White, PieceType.Pawn)
    else if (nwRaw & mask) != 0L then Piece.someOf(Color.White, PieceType.Knight)
    else if (bwRaw & mask) != 0L then Piece.someOf(Color.White, PieceType.Bishop)
    ...
    else if (nbRaw & mask) != 0L then Piece.someOf(Color.Black, PieceType.Knight)
    else if (bbRaw & mask) != 0L then Piece.someOf(Color.Black, PieceType.Bishop)
  ...
  /** Overwrite all bitboards (incl. aggregates) from another board. The
    * `from` is the parent position; `o` is read through [[BoardLike]] so
    * the source can be either a [[chess.model.board.BoardState]] (root
    * load) or another [[MutableBoard]] (parent→child). */
  ...
  /** Clear `fromIdx` and `toIdx` across all twelve bitboards (removing
    * the moving piece and any piece captured on `toIdx`), then set the
    * piece identified by `pieceIdx` (0..11, white P,N,B,R,Q,K then black)
    * on `toIdx`. Same one-shot bit logic as `BoardState.movePiece`,
    * mutating in place. `pieceIdx` is the POST-promotion piece for a
    * promoting pawn. Aggregates are NOT refreshed here — the caller
    * batches a single [[recomputeAggregates]] after all sub-moves
    * (castling rook hop, en-passant clear) are applied. */
  ...
  /** Clear `idx` across all twelve bitboards. Used to remove the pawn
    * captured en passant (it sits on a different square than `to`). */
