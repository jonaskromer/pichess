...
object SearchRegressionSpec extends ZIOSpecDefault:
  ...
  private def mv(from: String, to: String): Move =
    Move(
      Position(from(0), from(1).asDigit),
      Position(to(0), to(1).asDigit),
      None
    )
  ...
  def spec = suite("SearchRegressionSpec")(
    ...
    test(
      "bestMove is deterministic across diverse positions (move-loop + history guard)"
    ) {
      for
        start <- FenParserRegex.parse(
          "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1"
        )
        kiwi <- FenParserRegex.parse(
          "r3k2r/p1ppqpb1/bn2pnp1/3PN3/1p2P3/2N2Q1p/PPPBBPPP/R3K2R w KQkq - 0 1"
        )
        tactic <- FenParserRegex.parse(
