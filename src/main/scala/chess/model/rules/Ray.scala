...
object Ray:
  ...
  val table: Map[PieceType, List[Ray]] = Map(
    PieceType.King -> allDirections.map((dc, dr) => Ray(dc, dr, 1, slides = true)),
    PieceType.Queen -> allDirections.map((dc, dr) => Ray(dc, dr, 7, slides = true)),
    PieceType.Rook -> orthogonal.map((dc, dr) => Ray(dc, dr, 7, slides = true)),
    PieceType.Bishop -> diagonal.map((dc, dr) => Ray(dc, dr, 7, slides = true)),
    PieceType.Knight -> knightOffsets.map((dc, dr) => Ray(dc, dr, 1, slides = false))
  )

  /** Walk a ray from `origin`, returning all reachable squares (stopping at first occupied). */
  def walk(board: Board, origin: Position, ray: Ray): List[Position] =
    def loop(col: Int, row: Int, step: Int, acc: List[Position]): List[Position] =
  ...
  def canReach(board: Board, from: Position, pieceType: PieceType, target: Position): Boolean =
