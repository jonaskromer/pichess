...
/** Equivalence + smoke tests for [[ArrayTaperedEvaluator]].
  *
  * The array-backed evaluator is a drop-in replacement for [[TaperedEvaluator]]
  * on the search hot path. The two must return identical scores for the same
  * weights + position; this spec pins that across a representative position
  * spread (start / mid-game / Kiwipete / endgame), under both fully-tapered and
  * legacy un-suffixed weight snapshots.
  */
object ArrayTaperedEvaluatorSpec extends ZIOSpecDefault:
  ...
  def spec = suite("ArrayTaperedEvaluator")(
    suite("equivalence with TaperedEvaluator")(
      test(
        "legacy un-suffixed weights produce the same score on every fixture"
      ) {
