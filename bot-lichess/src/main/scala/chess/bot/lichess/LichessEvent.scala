...
/** Discriminated unions modelling the events the Lichess Bot API emits.
  ...
  *     [[GameEvent]] envelopes — initial game state plus subsequent
  *     state updates and chat lines.
  *
  * Lichess uses a `"type": "..."` discriminator. zio-json's
  * `@jsonDiscriminator` matches that shape directly so the JSON parses
  * straight into these ADTs without a custom decoder.
  */
...
object AccountEvent:
  ...
  /** Sent on every game state change for an active game — used to
    * track when our turn rolls around, when the game ends, etc. We
    * only need the id to subscribe to the per-game stream. */
  @jsonHint("gameStart")
  final case class GameStart(game: GameRef) extends AccountEvent

  /** Sent once when a game finishes (any reason). Triggers cleanup of
    * the per-game fiber. */
  @jsonHint("gameFinish")
  final case class GameFinish(game: GameRef) extends AccountEvent

  /** Incoming challenge — the bot decides whether to accept based on
    * its policy (variant, time control, rating range, …). */
  @jsonHint("challenge")
  ...
  final case class ChallengeCanceled(challenge: ChallengeInfo) extends AccountEvent

  @jsonHint("challengeDeclined")
  final case class ChallengeDeclined(challenge: ChallengeInfo) extends AccountEvent
...
/** Minimal game-identifying payload that arrives nested under the
  * `game` field of [[AccountEvent.GameStart]] / [[AccountEvent.GameFinish]].
  * We only need the id to hop to the per-game stream. */
...
/** Challenge payload. `variant.key == "standard"` is the only variant
  * our bot accepts in Phase 2 — everything else (chess960, atomic,
  * antichess, …) gets declined to keep the scope tight. */
...
object GameEvent:
  ...
  /** The very first event on every per-game stream: full game header
    * + current state. `initialFen == "startpos"` for the standard
    * starting position; any other value is a FEN to parse. `moves`
    * is the space-separated UCI history (may be empty for a fresh game).
    *
    * `state` is nested rather than flattened because Lichess emits the
    * GameState as a sub-document. We mirror that shape so the decoder
  ...
  /** State update on every move (ours or theirs). `moves` is the full
    * UCI history (cumulative — Lichess re-sends the whole list each
    * tick). `status` carries the game lifecycle ("started", "mate",
    * "draw", "resign", …). */
  @jsonHint("gameState")
  ...
  @jsonHint("chatLine")
  final case class ChatLine(room: String, username: String, text: String) extends GameEvent

  /** Opponent disconnected; Lichess auto-pauses the game. No action
    * needed on our side. */
...
/** Nested inside [[GameEvent.GameFull.state]]. Same shape as the
  * top-level [[GameEvent.GameStateEvent]] but discriminator-less since
  * it's never the root of an event envelope. */
