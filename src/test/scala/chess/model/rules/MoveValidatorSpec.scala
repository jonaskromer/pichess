  ...
  def spec = suite("MoveValidator")(
    ...
    suite("white pawn")(
      test("allow one square forward") {
        ...
        MoveValidator.validate(s, Move(pos('e', 2), pos('e', 3))).as(assertTrue(true))
      },
      test("allow two squares forward from rank 2") {
        val s = state(pos('e', 2) -> WP)
        MoveValidator.validate(s, Move(pos('e', 2), pos('e', 4))).as(assertTrue(true))
      },
      test("reject two-square advance from non-starting rank") {
        val s = state(pos('e', 3) -> WP)
        for exit <- MoveValidator.validate(s, Move(pos('e', 3), pos('e', 5))).exit
        yield assertTrue(exit.isFailure)
      },
      test("reject two-square advance when intermediate square is blocked") {
        val s = state(pos('e', 2) -> WP, pos('e', 3) -> BP)
        for exit <- MoveValidator.validate(s, Move(pos('e', 2), pos('e', 4))).exit
        yield assertTrue(exit.isFailure)
      },
      test("reject two-square advance when destination is occupied but path is clear") {
      ...
      test("reject move forward when destination is occupied") {
        ...
        for exit <- MoveValidator.validate(s, Move(pos('e', 3), pos('e', 4))).exit
        yield assertTrue(exit.isFailure)
      },
      test("allow diagonal capture of enemy piece") {
        val s = state(pos('e', 3) -> WP, pos('f', 4) -> BP)
        MoveValidator.validate(s, Move(pos('e', 3), pos('f', 4))).as(assertTrue(true))
      },
      test("reject diagonal move to empty square") {
        val s = state(pos('e', 3) -> WP)
        for exit <- MoveValidator.validate(s, Move(pos('e', 3), pos('f', 4))).exit
        yield assertTrue(exit.isFailure)
      },
      test("reject moving backward") {
        val s = state(pos('e', 3) -> WP)
        for exit <- MoveValidator.validate(s, Move(pos('e', 3), pos('e', 2))).exit
        yield assertTrue(exit.isFailure)
      },
      test("reject moving sideways") {
        val s = state(pos('e', 3) -> WP)
        for exit <- MoveValidator.validate(s, Move(pos('e', 3), pos('f', 3))).exit
        yield assertTrue(exit.isFailure)
      }
    ),
    // ─── Pawn (Black) ───────────────────────────────────────────────────────────
    suite("black pawn")(
      test("allow one square forward (toward rank 1)") {
        ...
        MoveValidator.validate(s, Move(pos('e', 7), pos('e', 6))).as(assertTrue(true))
      },
      test("allow two squares forward from rank 7") {
        val s = blackState(pos('e', 7) -> BP)
        MoveValidator.validate(s, Move(pos('e', 7), pos('e', 5))).as(assertTrue(true))
      },
      test("reject two-square advance from non-starting rank") {
        val s = blackState(pos('e', 6) -> BP)
        for exit <- MoveValidator.validate(s, Move(pos('e', 6), pos('e', 4))).exit
        yield assertTrue(exit.isFailure)
      },
      test("allow diagonal capture of enemy piece") {
        val s = blackState(pos('e', 7) -> BP, pos('f', 6) -> WP)
        MoveValidator.validate(s, Move(pos('e', 7), pos('f', 6))).as(assertTrue(true))
      }
    ),
    // ─── Rook ───────────────────────────────────────────────────────────────────
    suite("rook")(
      test("allow horizontal move on a clear rank") {
        ...
        MoveValidator.validate(s, Move(pos('a', 1), pos('h', 1))).as(assertTrue(true))
      },
      test("allow vertical move on a clear file") {
        val s = state(pos('a', 1) -> WR)
        MoveValidator.validate(s, Move(pos('a', 1), pos('a', 8))).as(assertTrue(true))
      },
      test("reject diagonal move") {
        val s = state(pos('a', 1) -> WR)
        for exit <- MoveValidator.validate(s, Move(pos('a', 1), pos('b', 2))).exit
        yield assertTrue(exit.isFailure)
      },
      test("reject knight-leap shape") {
        val s = state(pos('a', 1) -> WR)
        for exit <- MoveValidator.validate(s, Move(pos('a', 1), pos('c', 2))).exit
        yield assertTrue(exit.isFailure)
      },
      test("reject irregular direction") {
        val s = state(pos('a', 1) -> WR)
        for exit <- MoveValidator.validate(s, Move(pos('a', 1), pos('c', 4))).exit
        yield assertTrue(exit.isFailure)
      },
    ...
    suite("bishop")(
      test("allow diagonal move on a clear path") {
        ...
        MoveValidator.validate(s, Move(pos('a', 1), pos('d', 4))).as(assertTrue(true))
      },
      test("allow diagonal move in the other direction") {
        val s = state(pos('d', 4) -> WB)
        MoveValidator.validate(s, Move(pos('d', 4), pos('a', 1))).as(assertTrue(true))
      },
      ...
      test("reject vertical move") {
        val s = state(pos('a', 1) -> WB)
        for exit <- MoveValidator.validate(s, Move(pos('a', 1), pos('a', 8))).exit
        yield assertTrue(exit.isFailure)
      },
      test("reject knight-leap shape") {
        val s = state(pos('a', 1) -> WB)
      ...
      test("reject move to own square (zero move)") {
        val s = state(pos('d', 4) -> WB)
        for exit <- MoveValidator.validate(s, Move(pos('d', 4), pos('d', 4))).exit
        yield assertTrue(exit.isFailure)
      },
      test("reject move when diagonal path is blocked") {
        val s = state(pos('a', 1) -> WB, pos('b', 2) -> BP)
        for exit <- MoveValidator.validate(s, Move(pos('a', 1), pos('d', 4))).exit
        yield assertTrue(exit.isFailure)
      }
    ),
    // ─── Queen ──────────────────────────────────────────────────────────────────
    suite("queen")(
      test("allow horizontal move") {
        ...
        MoveValidator.validate(s, Move(pos('d', 1), pos('h', 1))).as(assertTrue(true))
      },
      test("allow vertical move") {
        val s = state(pos('d', 1) -> WQ)
        MoveValidator.validate(s, Move(pos('d', 1), pos('d', 8))).as(assertTrue(true))
      },
      test("allow diagonal move") {
        val s = state(pos('d', 1) -> WQ)
        MoveValidator.validate(s, Move(pos('d', 1), pos('g', 4))).as(assertTrue(true))
      },
      test("reject knight-leap shape") {
        val s = state(pos('d', 1) -> WQ)
        for exit <- MoveValidator.validate(s, Move(pos('d', 1), pos('f', 2))).exit
        yield assertTrue(exit.isFailure)
      },
      test("reject irregular direction") {
        val s = state(pos('d', 1) -> WQ)
        for exit <- MoveValidator.validate(s, Move(pos('d', 1), pos('e', 3))).exit
        yield assertTrue(exit.isFailure)
      },
      test("reject move to own square (zero move)") {
        val s = state(pos('d', 1) -> WQ)
        for exit <- MoveValidator.validate(s, Move(pos('d', 1), pos('d', 1))).exit
        yield assertTrue(exit.isFailure)
      },
      test("reject move when path is blocked") {
        val s = state(pos('d', 1) -> WQ, pos('f', 1) -> BP)
        for exit <- MoveValidator.validate(s, Move(pos('d', 1), pos('h', 1))).exit
        yield assertTrue(exit.isFailure)
      }
    ),
    // ─── Knight ─────────────────────────────────────────────────────────────────
    suite("knight")(
      test("allow L-shape (2 forward, 1 side)") {
        ...
        MoveValidator.validate(s, Move(pos('g', 1), pos('f', 3))).as(assertTrue(true))
      },
      test("allow L-shape (1 forward, 2 side)") {
        val s = state(pos('g', 1) -> WN)
        MoveValidator.validate(s, Move(pos('g', 1), pos('h', 3))).as(assertTrue(true))
      },
      test("allow L-shape (2 side, 1 forward)") {
        val s = state(pos('b', 1) -> WN)
        MoveValidator.validate(s, Move(pos('b', 1), pos('d', 2))).as(assertTrue(true))
      },
      ...
      test("reject straight move") {
        val s = state(pos('g', 1) -> WN)
        for exit <- MoveValidator.validate(s, Move(pos('g', 1), pos('g', 3))).exit
        yield assertTrue(exit.isFailure)
      },
      test("reject diagonal one step") {
        val s = state(pos('g', 1) -> WN)
        for exit <- MoveValidator.validate(s, Move(pos('g', 1), pos('h', 2))).exit
        yield assertTrue(exit.isFailure)
      },
      test("reject horizontal move") {
    ...
    suite("king")(
      test("allow one square horizontally") {
        ...
        MoveValidator.validate(s, Move(pos('e', 1), pos('f', 1))).as(assertTrue(true))
      },
      test("allow one square vertically") {
        val s = state(pos('e', 1) -> WK)
        MoveValidator.validate(s, Move(pos('e', 1), pos('e', 2))).as(assertTrue(true))
      },
      test("allow one square diagonally") {
        val s = state(pos('e', 1) -> WK)
        MoveValidator.validate(s, Move(pos('e', 1), pos('f', 2))).as(assertTrue(true))
      },
      test("reject two squares") {
        val s = state(pos('e', 1) -> WK)
        for exit <- MoveValidator.validate(s, Move(pos('e', 1), pos('g', 1))).exit
        yield assertTrue(exit.isFailure)
      },
      test("reject knight-leap shape") {
        val s = state(pos('e', 1) -> WK)
        for exit <- MoveValidator.validate(s, Move(pos('e', 1), pos('g', 2))).exit
        yield assertTrue(exit.isFailure)
      },
      test("reject move to own square (zero move)") {
        val s = state(pos('e', 1) -> WK)
        for exit <- MoveValidator.validate(s, Move(pos('e', 1), pos('e', 1))).exit
        yield assertTrue(exit.isFailure)
      }
    ),
    // ─── Own piece capture ──────────────────────────────────────────────────────
    ...
    suite("en passant")(
      test("allow white pawn en passant capture") {
        ...
        MoveValidator.validate(s, Move(pos('e', 5), pos('d', 6))).as(assertTrue(true))
      },
      test("allow black pawn en passant capture") {
        ...
        MoveValidator.validate(s, Move(pos('d', 4), pos('e', 3))).as(assertTrue(true))
      },
      test("reject diagonal to empty square without en passant target") {
        val s = state(pos('e', 5) -> WP)
        for exit <- MoveValidator.validate(s, Move(pos('e', 5), pos('d', 6))).exit
        yield assertTrue(exit.isFailure)
      },
    ...
    suite("source and turn")(
      test("error includes source position when square is empty") {
        val s = state(pos('a', 1) -> WR)
        for err <- MoveValidator.validate(s, Move(pos('e', 4), pos('e', 5))).flip
