...
enum GameError(val message: String) extends Exception(message):
  ...
  /** Transport / infrastructure failure talking to a downstream service —
    * e.g. the repository microservice is unreachable or returned 5xx. Kept
    * distinct from [[ParseError]] so retry policies can target it.
    */
