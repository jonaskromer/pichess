...
/** Smoke tests for the gateway's REST surface. The gateway is wired against the
  * gameService gRPC contract; the server side is the real `GrpcServer` impl
  * bound to an in-process gRPC transport so tests don't need a real port. The
  * client is the generated `GameServiceClient` connected via the same
  * in-process channel — exactly the production code path, just without a
  * network. Each test gets a fresh server (UUID-named) so parallel execution
  * doesn't collide on the in-process registry.
  */
object WebControllerRoutesSpec extends ZIOSpecDefault:
  ...
  private def runWith[A](
      stackInfo: chess.controller.StackInfo =
      ...
      out <- ZIO.scoped {
        (for
          ...
          routes = WebController.routes(
            ...
            tournamentBaseUrl = "http://tournament:8086",
            botControlUrl = "http://bot-tournament:8080",
            botName = "pichess",
  ...
  def spec = suite("WebController routes")(
    ...
    test(
      "POST /api/games/{id}/move from a different session returns 400 (forbidden)"
    ) {
      runWith() { routes =>
        for
    ...
    test("multiple games run side by side without interfering") {
      runWith() { routes =>
        for
          ...
          s1 <- routes
            .runZIO(Request.get(url"/api/games/$g1/state"))
            .flatMap(_.body.asString)
          s2 <- routes
            .runZIO(Request.get(url"/api/games/$g2/state"))
            .flatMap(_.body.asString)
        yield assertTrue(
          g1 != g2,
    ...
    test(
      "GET /api/games/{id}/attackers?of=e4 is empty in the initial position"
    ) {
      runWith() { routes =>
        for
          id <- createGame(routes)
    ...
    test(
      "GET /api/state with ?format=fen returns the export payload (StateResponse.Export branch)"
    ) {
      runWith() { routes =>
        for
          id <- createGame(routes)
          response <- routes.runZIO(
            Request.get(url"/api/games/$id/state?format=fen")
          )
          body <- response.body.asString
        yield assertTrue(
          response.status == Status.Ok,
    ...
    test(
      "POST /api/games with `load` invokes the gameService loadGame branch"
    ) {
    ...
    test(
      "POST /api/games/{id}/undo invalidates the annotation cache (success path)"
    ) {
      ...
      runWith() { routes =>
        for
          ...
          _ <- routes.runZIO(
            Request.get(url"/api/games/$id/legal-moves?from=e4")
          )
          undo <- routes.runZIO(
    ...
    test(
      "POST /api/games/{id}/draw returns 400 when no claim condition holds (failure path)"
    ) {
      runWith() { routes =>
        for
          id <- createGame(routes)
    ...
    test(
      "POST /api/games/{id}/forfeit invokes the gameService forfeit branch (cache invalidate)"
    ) {
    ...
    test("GET /web/<missing> returns 404 (unknown extension)") {
      runWith() { routes =>
        for response <- routes.runZIO(Request.get(url"/web/does-not-exist.xyz"))
        yield assertTrue(response.status == Status.NotFound)
      }
    },
    test("GET /web/<path traversal> returns 404 (isSafeAssetPath blocks ..)") {
      runWith() { routes =>
        for response <- routes.runZIO(Request.get(url"/web/../etc/passwd"))
        yield assertTrue(response.status == Status.NotFound)
      }
    },
    test(
      "GET /dev/coverage/report/ returns 404 when PICHESS_DEV is unset (default)"
    ) {
      runWith() { routes =>
    ...
    test(
      "GET /dev/coverage/report/<missing> returns 404 when dev mode IS enabled but the file isn't present"
    ) {
    ...
    test(
      "POST /api/games with malformed `load` returns 400 (loadGame error path)"
    ) {
    ...
    test(
      "two consecutive legal-moves calls hit the annotation cache (cached branch)"
    ) {
    ...
    test(
      "GET /api/games/{id}/export/fen returns 404 for an unknown game (export error path)"
    ) {
    ...
    test(
      "GET /api/games/{id}/state returns 400 for an unknown game (currentBoard error path)"
    ) {
    ...
    test(
      "state after a black move includes both colors in the move log (parseColor White + Black)"
    ) {
    ...
    test("GET /web/<svg> returns image/svg+xml (contentTypeFor .svg branch)") {
      runWith() { routes =>
        for response <- routes.runZIO(Request.get(url"/web/missing.svg"))
        yield assertTrue(
    ...
    test("GET /web/<png> traverses the .png arm of contentTypeFor") {
      runWith() { routes =>
        for response <- routes.runZIO(Request.get(url"/web/missing.png"))
        yield assertTrue(
          response.status == Status.Ok || response.status == Status.NotFound
        )
      }
    ...
    test("GET /web/<html> traverses the .html arm of contentTypeFor") {
      runWith() { routes =>
        for response <- routes.runZIO(Request.get(url"/web/missing.html"))
        yield assertTrue(
          response.status == Status.Ok || response.status == Status.NotFound
        )
      }
    ...
    test("GET /web/<json> traverses the .json arm of contentTypeFor") {
      runWith() { routes =>
        for response <- routes.runZIO(Request.get(url"/web/missing.json"))
        yield assertTrue(
          response.status == Status.Ok || response.status == Status.NotFound
        )
      }
    ...
    test("GET /web/<js> traverses the .js arm of contentTypeFor") {
      runWith() { routes =>
        for response <- routes.runZIO(Request.get(url"/web/missing.js"))
        yield assertTrue(
          response.status == Status.Ok || response.status == Status.NotFound
        )
      }
    ...
    test(
      "GET /dev/coverage/report/ (no trailing path) rewrites to index.html"
    ) {
    ...
    test(
      "GET /dev/coverage/report/<no-extension> hits the case _ 404 branch of serveDevAsset"
    ) {
    ...
    test(
      "POST /api/games/{id}/draw succeeds at halfmove=100 (50-move rule) and invalidates the cache"
    ) {
    ...
    test(
      "GET /api/games/{id}/threats with an actively threatened piece returns a non-empty list (covers attackerEntries map)"
    ) {
    ...
    test(
      "GET /api/games/missing/legal-moves returns 400 (annotationsFor toErrorDto path)"
    ) {
    ...
    test(
      "GET /web/<file_with_underscore>.css traverses the `_` arm of isSafeAssetPath"
    ) {
      runWith() { routes =>
        for response <- routes.runZIO(Request.get(url"/web/foo_bar.css"))
        yield assertTrue(
    ...
    test(
      "GET /api/games/{id}/events opens an SSE stream (serveEvents handler)"
    ) {
