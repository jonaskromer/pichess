...
/** Pure per-game decision logic.
  *
  * Each Lichess [[GameEvent]] is handed to [[decide]] alongside the bot's
  * username and the running state (color, initial position). The result is a
  * tuple of the next running state and an [[Action]] the orchestrator should
  * perform. The split keeps the I/O-free choice logic unit-testable end-to-end
  * without needing a real Lichess connection or a stubbed sttp backend.
  *
...
object GameRunner:
  ...
  /** Stable per-game context discovered from the initial [[GameEvent.GameFull]]
    ...
    * deltas can reconstruct the current position via "initial FEN + cumulative
    * move list" without re-parsing the GameFull payload.
    */
  ...
  /** Drive one event. Returns the next [[State]] (may differ on GameFull where
    * we discover our color) and an [[Action]] that the orchestrator interprets.
    */
  def decide(
      event: GameEvent,
      botUsername: String,
  ...
  /** Process the initial gameFull envelope: discover our color, parse the
    * initial FEN, derive the current state from initialFen + state.moves, and
      ...
      case Right(ourColor) =>
        ...
        deriveState(event.initialFen, event.state.moves) match
          ...
          case Right(current) =>
            ...
            (
              Some(runState),
              turnAction(current, ourColor, s.wtime, s.btime, s.winc, s.binc)
            )

  /** Process a per-move state update — same shape as the GameFull substate, but
    * on a stream that pre-supposes the GameFull arrived.
    */
  ...
  /** Recompute the live `GameState` from initialFen + the cumulative UCI move
    * list Lichess includes on every state event. The re-derivation is
    * sub-millisecond at typical game depths, and keeps us stateless across
    * reconnects (Lichess re-sends the same cumulative moves string on resume).
    */
  private def deriveState(
      initialFen: String,
      movesUci: String
  ...
  /** Determine which colour the bot is playing by matching the `botUsername`
    * against the white/black PlayerRef names. Returns Left when the bot isn't
    * listed at all — that's a malformed event since Lichess only streams events
    * for games the bot is in.
    */
  ...
  /** Map the current GameState's active colour against our colour: if they line
    * up it's our turn → [[Action.MoveFrom]]; otherwise we wait →
    * [[Action.None]].
    */
  private def turnAction(
    ...
    if current.activeColor == ourColor then
      val (ourTime, ourInc, oppTime) =
        if ourColor == Color.White then (wtime, winc, btime)
        else (btime, binc, wtime)
      Action.MoveFrom(current, ourTime, ourInc, oppTime)
    else Action.None

  /** Lichess' status string is "created" before first move or "started" while
    * in play; anything else (mate / draw / resign / stalemate / timeout /
    * aborted) means the game ended.
    */
