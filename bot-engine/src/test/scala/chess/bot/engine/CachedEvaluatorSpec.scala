...
object CachedEvaluatorSpec extends ZIOSpecDefault:
  ...
  def spec = suite("CachedEvaluator + incremental")(
    test(
      "passes incrementalNet through (wrapping in the cache must NOT disable incremental)"
    ) {
      assertTrue(CachedEvaluator.of(new Counting).incrementalNet.isDefined)
    },
    test(
      "caches the evaluateWith path — a transposition hits the cache (one inner call)"
    ) {
