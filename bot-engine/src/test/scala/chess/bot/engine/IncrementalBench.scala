...
/** Throwaway wall-clock benchmark: time-to-fixed-depth with the incremental
  * NNUE accumulator ON vs OFF (full rebuild). Same search, same result (see
  * IncrementalSearchSpec) — the ratio is the eval-speed win. Run: `sbt
  * 'botEngine/Test/runMain chess.bot.engine.IncrementalBench'`
  */
  ...
  def main(args: Array[String]): Unit =
    ...
    def parse(fen: String): GameState =
      Unsafe.unsafe(implicit u =>
        rt.unsafe.run(FenParserRegex.parse(fen)).getOrThrow()
      )
    def mk(inc: Boolean): Search =
      new AlphaBetaSearch(
        eval = eval,
        tt = TranspositionTable.inMemory(1 << 20),
        book = OpeningBook.Empty,
        incrementalAccumulators = inc
      )
    def run(s: Search, st: GameState, depth: Int): Unit =
      Unsafe.unsafe(implicit u =>
        rt.unsafe.run(s.bestMove(st, depth)).getOrThrow()
      ): Unit
    ...
    for d <- Seq(7, 8) do
      ...
      println(
        f"[bench] depth $d: full-rebuild=${full}ms  incremental=${incr}ms  speedup=${full.toDouble / math.max(1, incr)}%.2fx"
      )
