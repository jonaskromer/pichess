...
/** All-in-one engine bootstrap.
  *
  * The bot's runtime is two pieces of data — weights + opening book — plus the
  * immutable code. [[EngineBundle]] is the single call that loads both from
  * committed classpath resources and assembles a ready-to-use [[Search]]. No
  * DB, no network, no external paths.
  *
  ...
  * }}}
  *
  * A second factory ([[fromResourcesOrFallback]]) returns the material-only
  * engine if either resource fails to load — useful for tooling that needs to
  * keep working when a bad weights snapshot is checked in by mistake.
  */
...
object EngineBundle:
  ...
  /** Which evaluator to use under the search. [[Hybrid]] is the default (it
    * measured +424 Elo vs pure HCE with the Stockfish- distilled net); the
    * other variants opt in. All non-HCE variants fall back to pure HCE when
    * their NNUE resource is missing, so the bundle always loads.
    */
  enum EvalSource:
    /** Hand-crafted eval — the array-backed tapered evaluator over the tuned
      * `weights/v{n}.json` snapshot. Same evaluator that has been running in
      * tournaments to date.
      */
    case Hce

    /** Single NNUE network loaded from `/nnue-v1.bin`. Falls back to HCE if the
      * resource is missing.
      */
    case Nnue

    /** NNUE ensemble — `/nnue-ens-v1-s{1..k}.bin`. K members average their
      * evals (variance reduction). Falls back to single NNUE then HCE if any
      * member is missing.
      */
    case NnueEns

    /** HCE + single-NNUE blend (see [[HybridEvaluator]]), mixed at
      * `hybridAlpha` (≈0.4 after the depth-6 re-tune). The strongest option:
      * the Stockfish-distilled NNUE is +17 Elo vs HCE standalone, and the
      * HCE-backbone-plus-NNUE-correction blend beats pure HCE by a wide margin.
      * Falls back to pure HCE if the NNUE resource is missing.
      */
    case Hybrid

  /** Default bundle: load `weights/v1.json` + `openings/main-lines.pgn` from
    * the classpath, assemble [[Search]] over those. Fails fast if either
    * resource is missing or malformed — bot startup shouldn't silently continue
    * with a half-loaded engine.
    *
    * Eval-source choice + eval cache are opt-in: defaults match the historical
    * HCE-no-cache behaviour so existing benches stay apples-to-apples until
    * callers explicitly switch.
    */
  def fromResources(
  ...
  /** Pick the requested evaluator with NNUE fallbacks, then layer the optional
    * endgame patch + Zobrist-keyed cache decorators.
    */
    ...
    val chosen = source match
      ...
      case EvalSource.Hybrid =>
        ...
        NnueEvaluator
          ...
          .map(nnue =>
            new HybridEvaluator(hce, nnue, hybridAlpha, hybridAlphaEndgame)
          )
          .getOrElse(hce)
    val withEndgame =
      if endgameHeuristics then new EndgameAwareEvaluator(chosen) else chosen
    if cacheEnabled then CachedEvaluator.of(withEndgame, cacheEntries)
    else withEndgame

  /** Same as [[fromResources]] but on any failure, falls back to the
    * material-only evaluator + empty opening book. Returns the bundle along
    * with the failure (if any) so the caller can log it.
    */
        ...
        ZIO.succeed(
          (
            EngineBundle(
              ...
              search =
                Search.alphaBeta(fbEval, OpeningBook.Empty, maxTtEntries),
              eval = fbEval
            ),
            Some(err)
          )
  ...
  /** Material-only seed weights, used as the in-code fallback when the JSON
    * resource is unreadable. Mirrors the hand-coded values in
    * [[MaterialEvaluator]].
    */
