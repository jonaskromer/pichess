...
/** Reverse proxy for the lobby-service.
  *
  * The web-ui is served from the gateway origin (`http://gateway:8090`), so any
  * cross-origin call to the lobby-service on `:8092` would trip the browser's
  * CORS preflight machinery. Routing every lobby request through the gateway
  * keeps everything same-origin: the browser never sends an OPTIONS preflight,
  * and the lobby-service no longer has to advertise permissive CORS headers.
  *
  * The proxy is dumb on purpose — it forwards method, path, query string,
  * headers and body verbatim, and streams the response back. All authentication
  * / rate-limiting hooks belong here later, but for now the gateway and the
  * lobby-service trust each other.
  */
object LobbyProxy:
  ...
  /** Normalise the trailing-path segment so it always starts with a single `/`.
    * zio-http currently hands us a `Path` whose `toString` has no leading
    * slash, but a future bump might change that — the guard keeps us correct
    * either way.
    */
  ...
  /** Merge tracing carrier entries into `base`. Extracted from `forward` so the
    * per-entry branch is unit-testable without standing up a real OpenTelemetry
    * context (which `TracingLayer.noop` deliberately omits —
    * `tracing.injectSpan` writes no entries under noop).
    */
  ...
  /** Compose the upstream URL string + parse it. Pulled out of `forward` so the
    * `URL.decode` failure arm — which can be hit by passing a base URL with
    * control characters — is unit-testable without standing up a proxy.
    */
  ...
  def routes(
    ...
    /** Forward a request: same method, body, query string and headers, URL
      * rewritten onto the lobby-service host. `Host` and `Content-Length` are
      * dropped because the underlying HTTP client recomputes them. Outgoing
      * requests are wrapped in a CLIENT span and the current trace context is
      * injected as W3C `traceparent` headers so the lobby-service's server-side
      * middleware can pick up the same trace.
      */
    def forward(prefix: String)(
        rest: Path,
        req: Request
    ): ZIO[Client & Tracing & ContextStorage, Nothing, Response] =
          ...
          ZIO.serviceWithZIO[Tracing] { tracing =>
            tracing.span(spanName, SpanKind.CLIENT) {
              ...
              for
                ...
                injectedHeaders = addCarrierHeaders(
                  baseHeaders,
  ...
  /** Read the base URL from the env var, falling back to the docker compose
    * default. Pure config — does no network I/O.
    */
