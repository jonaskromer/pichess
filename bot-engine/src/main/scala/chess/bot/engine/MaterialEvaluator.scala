...
/** Material-only evaluator. See [[Evaluator.materialOnly]] for context.
  *
  * Counted directly from the 12 per-piece bitboards via `popCount` —
  * no iteration, ~12 bitcount instructions per call. Centipawn values
  * are the Kaufman / starter-engine standard set.
  */
