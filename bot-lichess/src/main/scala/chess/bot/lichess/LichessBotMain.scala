...
/** Runnable entrypoint for the Lichess bot.
  *
  * Loads the strongest engine configuration and plays every accepted game with
  * time-budgeted iterative deepening:
  *
  *   - Eval: the HCE+NNUE **Hybrid** (the strongest evaluator;
  *     `EvalSource.Hybrid` is `EngineBundle`'s default) over the **v8** tuned
  *     weights backbone (the champion HCE snapshot, +14 Elo over v4) blended
  *     with the Stockfish-distilled NNUE.
  *   - Search: [[Search.bestMoveWithBudget]] — iterative deepening to a
  *     per-move wall-clock budget. Stronger than a fixed depth (it goes deeper
  *     in quiet positions) and safe on the clock (it never blows the budget),
  *     so the bot won't flag.
  *
  * Config via env:
    ...
    for
      ...
      _ <- ZIO.scoped {
        HttpClientZioBackend.scoped().flatMap { backend =>
          ...
          val tbOracle =
            Option.when(tablebase)(new LichessTablebaseSearch(backend))
          val searchFactory = () =>
            ...
            tbOracle.fold(base)(o =>
              new TbAugmentedSearch(base, o, TablebasePieceLimit)
            )
          Bridge
            .run(username, searchFactory, fallbackDepth, api)
            .tapError(e =>
              ZIO.logError(
                s"Lichess event stream failed, reconnecting in 5s: ${e.getMessage}"
              )
            )
