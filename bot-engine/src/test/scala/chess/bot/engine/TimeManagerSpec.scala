...
object TimeManagerSpec extends ZIOSpecDefault:
  ...
  def spec = suite("TimeManager.budgetMs")(
    ...
    test("NEVER spends past the safety buffer (no flagging)") {
      ...
      val checks =
        for r <- List(150L, 300L, 1_000L, 5_000L, 60_000L)
        yield TimeManager.budgetMs(
          r,
          0
        ) <= r - TimeManager.SafetyBufferMs || r <= TimeManager.SafetyBufferMs + TimeManager.MinBudgetMs
    ...
    test("more time → at least as much budget (monotonic)") {
      assertTrue(
        TimeManager.budgetMs(300_000, 0) >= TimeManager.budgetMs(60_000, 0)
      )
    },
    ...
    test(
      "a large increment is still bounded by the share cap (won't blow the clock)"
    ) {
    ...
    test("middlegame gets more search time than the opening (same clock)") {
      val mid = TimeManager
        .budgetMs(120_000, 2_000, 120_000, phase = 0.5, inCheck = false)
      val open = TimeManager
        .budgetMs(120_000, 2_000, 120_000, phase = 1.0, inCheck = false)
      assertTrue(mid > open)
    },
    test("ahead on the clock spends more than behind (same position)") {
      val ahead = TimeManager.budgetMs(
        120_000,
        2_000,
        oppRemainingMs = 20_000,
        phase = 1.0,
        inCheck = false
      )
      val behind = TimeManager.budgetMs(
        120_000,
        2_000,
        oppRemainingMs = 600_000,
        phase = 1.0,
        inCheck = false
      )
    ...
    test("a check is given more time than a quiet position") {
      val chk = TimeManager
        .budgetMs(120_000, 2_000, 120_000, phase = 0.5, inCheck = true)
      val quiet = TimeManager
        .budgetMs(120_000, 2_000, 120_000, phase = 0.5, inCheck = false)
