...
/** Direct unit tests for LobbyProxy's path/URL composition helpers. The
  * full-stack proxy spec exercises the happy path through a real upstream
  * server — the helpers here are the parts that can't be hit end-to-end
  * (zio-http always normalises Path so its `toString` is leading-slash-free;
  * `URL.decode` only fails on URL strings the proxy itself would never
  * construct from a well-formed inbound request).
  */
object LobbyProxyHelpersSpec extends ZIOSpecDefault:
  ...
  def spec = suite("LobbyProxy helpers")(
    ...
    suite("buildTarget")(
      ...
      test(
        "returns Left with the offending string when URL.decode rejects the result"
      ) {
