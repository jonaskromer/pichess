...
/** Search-internal MUTABLE position for copy-make: a [[MutableBoard]]
  * plus the position metadata that move-gen / Zobrist / eval read
  * ([[PositionView]]). Reused per ply via `SearchBufs.positions`, so
  * descending into a child allocates nothing — the per-node immutable
  * `GameState` (~18 %) + the apply `Some` (~13 %) both vanish.
  *
  * `enPassantTarget` / `castlingRights` are kept as the immutable
  * `Option[Position]` / `CastlingRights` the readers already consume:
  * they only churn on a double-push / a rights change (rare), so reads
  * stay allocation-free while matching `GameState` bit-for-bit.
  *
  * The copy-make apply ([[copyMakeInto]]) is a faithful in-place port of
  * `Game.applyMoveCoreSync` + `buildPostMoveState`: same board result,
  * same metadata, same legality verdict — verified by `PerftSpec` (the
  * published perft counts) and the determinism / incremental-NNUE
  * golden specs. */
private[engine] final class SearchPos extends PositionView:
  ...
  /** Load this position from an immutable one (the search root, ply 0).
    * `s` is typically a `GameState`; read through [[PositionView]]. */
  ...
  /** Copy-make: copy `this` into `child`, apply `moveInt` in place
    * (captures, castling rook hop, en passant, promotion incl.
    * under-promotion), flip the side to move, and update en-passant
    * target / castling rights / halfmove clock. Returns the move's
    * LEGALITY as a Boolean — `true` iff the move does not leave the
    * mover's king in check (no `Option`/`null`/sentinel). `child` MUST
    * be a different instance than `this`.
    *
    * Equivalent to `Game.applyMoveCoreSync(this, decode(moveInt))
    * .isDefined`, leaving the post-move position in `child`. */
  def copyMakeInto(child: SearchPos, moveInt: Int): Boolean =
    ...
    val typeOrd =
      if white then
        ...
        if      (b.pawnsB.raw   & fromMask) != 0L then 0
    ...
    if isCastling then
      ...
      cb.clearSquare(if white then toIdx - 8 else toIdx + 8)
  ...
  /** Copy `this` into `child` for a NULL move: same board, flipped
    * side, cleared en-passant target, +1 halfmove clock (castling
    * rights unchanged). Mirrors the old `nullMoveState`; the board is
    * untouched so the maintained NNUE accumulator stays valid. */
  def copyNullMoveInto(child: SearchPos): Unit =
    child.board.copyFrom(board)
    child.activeColor     = if activeColor == Color.White then Color.Black else Color.White
    child.enPassantTarget = None
    child.castlingRights  = castlingRights
    child.halfmoveClock   = halfmoveClock + 1

  /** Castling-rights after a move — port of `Game.updatedCastlingRights`
    * to LERF indices. Returns the SAME `CastlingRights` instance when no
    * flag changes (the common case) so quiet moves don't allocate. */
  private def nextCastlingRights(
    ...
    val fromFile = fromIdx & 7
    if typeOrd == 5 then
      if white then { wK = false; wQ = false } else { bK = false; bQ = false }
