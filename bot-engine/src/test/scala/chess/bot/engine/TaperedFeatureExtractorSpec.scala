...
object TaperedFeatureExtractorSpec extends ZIOSpecDefault:
  ...
  def spec = suite("TaperedFeatureExtractor.full")(
    ...
    test(
      "at full opening (phase = 1.0): all weight is on `_mg`, `_eg` is zero"
    ) {
    ...
    test(
      "at full endgame (phase = 0.0): all weight is on `_eg`, `_mg` is zero"
    ) {
    ...
    test("mid-phase position splits a feature across `_mg` and `_eg`") {
      ...
      yield
        ...
        val sum = tapered.getOrElse("tempo_mg", 0.0) + tapered.getOrElse(
          "tempo_eg",
          0.0
        )
