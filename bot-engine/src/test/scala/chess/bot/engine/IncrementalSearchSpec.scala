...
/** End-to-end correctness gate for the incremental NNUE accumulator: a
  * search that maintains the accumulator across make/unmake must return
  * **byte-identical** results — same bestMove AND same score, at the same
  * depth — as a search that rebuilds the accumulator from scratch each
  * leaf. Any wiring bug (a make-move site that doesn't advance/restore
  * the accumulator) desyncs it from the board, the leaf evals drift, and
  * the search picks a different move / reports a different score — caught
  * here. Exercised across quiet, tactical (captures/checks → quiescence)
  * and endgame positions so the wrapped paths (searchMoves stages, LMR
  * re-search, quiescence, null move) all fire. */
object IncrementalSearchSpec extends ZIOSpecDefault:
  ...
  private def matchesFullRebuild(eval: Evaluator): ZIO[Any, Throwable, TestResult] =
    val inc  = mk(eval, incremental = true)
    val full = mk(eval, incremental = false)
    ZIO
      .foreach(fens) { fen =>
        for
          state    <- FenParserRegex.parse(fen).mapError(e => new RuntimeException(e.toString))
