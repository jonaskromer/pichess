...
object TimeManager:
  ...
  /** Backward-compatible budget: no opponent clock, neutral stage, not in
    * check (reproduces the original conservative formula exactly). */
  def budgetMs(remainingMs: Long, incMs: Long): Long =
    budgetMs(remainingMs, incMs, oppRemainingMs = -1L, phase = 1.0, inCheck = false)

  /** Per-move search budget, adaptive to game stage, clock advantage, and
    * check — banked time is spent on the moves that actually decide games.
    *
    * @param remainingMs    our remaining clock, ms
    * @param incMs          our increment per move, ms
    * @param oppRemainingMs opponent's remaining clock, ms (<= 0 = unknown)
    * @param phase          game phase 1.0 (opening) .. 0.0 (bare endgame),
    *                       e.g. from [[chess.bot.engine.GamePhase]]
  ...
  def budgetMs(
    ...
    if safe <= MinBudgetMs then
      ...
      val stageMult = 1.0 + MiddlegameBoost * (1.0 - math.abs(phase - 0.5) * 2.0)
      val advMult   =
        if oppRemainingMs <= 0L then 1.0
        else clampD(remainingMs.toDouble / oppRemainingMs.toDouble, AdvMin, AdvMax)
