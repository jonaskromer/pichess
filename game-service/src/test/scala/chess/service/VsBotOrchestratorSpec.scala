...
/** Behavioural tests for the vs-bot orchestrator.
  *
  * The real material-only [[Search]] is used (cheap at depth 1) so
  * the bot's moves are concrete legal moves — tests verify the
  * orchestration sequencing without coupling to specific picks.
  */
object VsBotOrchestratorSpec extends ZIOSpecDefault:

  /** Test-shaped layer: GameService + in-memory bot-config repo +
    * a real (material-only) Search at low depth so tests are quick. */
  private val testLayer: ULayer[GameService & BotConfigRepository & VsBotOrchestrator] =
  ...
  def spec = suite("VsBotOrchestrator")(
    suite("newGameVsBot")(
      test("plays the opening move when the bot has white") {
        for
          ...
          gs   <- orch.newGameVsBot(BotConfig(
                    botSide    = Color.White,
    ...
    suite("makeMove")(
      test("player + bot reply when bot is on the active side after player moves") {
        for
          orch   <- ZIO.service[VsBotOrchestrator]
      ...
      test("surfaces InvalidMove when bot search returns None") {
        ...
        for
          ...
          exit        <- orch.newGameVsBot(BotConfig(
                           botSide    = Color.White,
                           difficulty = Difficulty.Beginner,
                           allowUndo  = true,
                         )).exit
