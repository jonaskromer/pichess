    ...
    suite("GameFull")(
      test("white pichess at startpos → MoveFrom (our turn)") {
        ...
        assertTrue(
          ...
          action match
            case GameRunner.Action.MoveFrom(s, _, _, _) => s.activeColor == Color.White
            case _ => false,
        )
      },
      ...
      test("black pichess after white's e2e4 → MoveFrom (our turn)") {
        ...
        assertTrue(
          next.exists(_.ourColor == Color.Black),
          action match
            case GameRunner.Action.MoveFrom(s, _, _, _) => s.activeColor == Color.Black
            case _ => false,
        )
      },
      ...
      test("name match is case-insensitive") {
        ...
        assertTrue(
          action match
            case GameRunner.Action.MoveFrom(_, _, _, _) => true
            case _ => false,
        )
      },
    ),
