...
/** Tests for the lobby reverse-proxy. The upstream is a tiny real zio-http
  * server bound to an ephemeral port (`Server.port = 0`); the proxy is
  * configured with that server's `http://localhost:<port>` base URL. Each
  * request through the proxy hits the real upstream and we assert on the
  * round-tripped response.
  *
  * The upstream is intentionally tiny — it echoes back the method, the path it
  * received, the query string, and any body. That's enough to verify
  * path/method/query/body forwarding without coupling the tests to the actual
  * lobby-service schema.
  */
object LobbyProxySpec extends ZIOSpecDefault:

  /** Echo upstream: responds to any /lobbies path with a body that captures
    * what it saw — `method path?query|body`.
    */
  private val upstream: Routes[Any, Response] = Routes(
    Method.ANY / "lobbies" / trailing -> handler { (rest: Path, req: Request) =>
  ...
  /** Bring up the upstream zio-http server on an ephemeral port, yield the
    * actual bound URL, then tear it down at scope exit. The proxy now wraps
    * outbound calls in a CLIENT span, so the test runtime needs `Tracing` &
    * `ContextStorage` — provided here as a noop so the upstream + proxy are
    * exercised without an OTLP exporter.
    */
    ...
    ZIO
      .scoped {
        ...
        ZIO
          ...
          .provideSomeLayer[Scope & Client & Tracing & ContextStorage](
            serverLayer
          )
      }
  ...
  def spec = suite("LobbyProxy")(
    ...
    test(
      "Host and Content-Length headers from the inbound request are dropped before forwarding"
    ) {
