...
/** Per-game spectator policy, handed to the gateway by the lobby when a hosted
  * game starts. Games with no registered policy (local / vs-bot / Lichess
  * mirror) are unrestricted. A non-positive `limit` means "no cap".
  */
final case class SpectatorPolicy(allowSpectate: Boolean, limit: Int)

/** Read-only view of a game's spectator state for the spectate index: its
  * policy (absent ⇒ unrestricted) and the current live viewer count.
  */
final case class SpectatorInfo(policy: Option[SpectatorPolicy], count: Int)

/** Why a spectator was refused. `code` is sent verbatim to the client as the
  * data of a `spectator-denied` SSE event.
  */
...
/** Live spectator presence + policy per game, derived purely from open SSE
  ...
  * occupies a slot for its lifetime (via [[admit]]) and every viewer — players
  * included — subscribes to [[changes]] to render a live count. [[setPolicy]]
  * records the lobby's allowSpectate + limit so [[admit]] can gate watchers.
  *
  * In-memory and unbounded over the process lifetime (one entry per game id
  * ever watched), mirroring game-service's own in-memory session map. A gateway
  * restart resets every count to zero and forgets policies, which is correct:
  * the SSE connections it was counting are gone too.
  */
final class SpectatorPresence(
  ...
  /** Get-or-create the count ref for a game. Serialised through the
    * `Ref.Synchronized` so two concurrent first-connectors can't each install a
    * fresh ref and lose one of the increments.
    */
  ...
  /** Read-only snapshot of a game's policy + current count for the spectate
    * index. Does NOT create a count ref (a never-watched game reads as 0), so
    * it's side-effect-free unlike [[changes]] / [[admit]].
    */
  def info(gameId: String): UIO[SpectatorInfo] =
    for
      policy <- policies.get.map(_.get(gameId))
      count <- refs.get.map(_.get(gameId)).flatMap {
        case Some(ref) => ref.get
        case None      => ZIO.succeed(0)
      }
    yield SpectatorInfo(policy, count)
  ...
  /** Seat a spectator for the lifetime of the calling scope, honoring the
    ...
    * concurrent spectators at the limit. The check-and-seat is atomic on the
    * count ref, so two simultaneous joins can't both slip past a limit of N.
    * Games without a policy are unrestricted. On success a slot is occupied and
    * released (decremented) when the scope closes.
    */
