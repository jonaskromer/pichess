...
/** Behavioural specs for the in-memory TT. Nothing search-specific — these pin
  * the store / load / cap / clear contract so the search can trust them.
  */
object TranspositionTableSpec extends ZIOSpecDefault:

  private val e1 = Entry(
    depth = 3,
    score = 42,
    kind = Kind.Exact,
  ...
  private val e2 =
    Entry(depth = 5, score = -7, kind = Kind.Lower, bestMove = None)
  ...
  def spec = suite("TranspositionTable")(
    ...
    test(
      "a colliding store replaces under the same depth policy, evicting the old key"
    ) {
      val tt = TranspositionTable.inMemory(maxEntries = 8)
    ...
    test(
      "depth-preferred: a shallower store does not overwrite a deeper entry"
    ) {
      val tt = TranspositionTable.inMemory(maxEntries = 8)
    ...
    test("aging lets a fresh, shallower entry replace a stale deeper one") {
      ...
      val shallow =
        Entry(depth = 2, score = 2, kind = Kind.Exact, bestMove = None)
      val tt = TranspositionTable.inMemory(maxEntries = 8)
